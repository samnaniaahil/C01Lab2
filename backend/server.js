    ...
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        if (data.deletedCount == 0) {
          return res
            .status(404)
            .json({ error: "Unable to find note with given ID for current user." });
        }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
    try {
      ...
      if (!title && !content) {
        return res
          .status(400)
          .json({ error: "At least one of title or content are required." });
      }
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        // Update note in database, if exists
        const collection = db.collection(COLLECTIONS.notes);
        var updates = {}
        if (!title) {
          ...
          updates = {content: content, title: title};
        }
